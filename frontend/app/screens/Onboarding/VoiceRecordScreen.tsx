...
type Phase = "idle" | "recording" | "uploading" | "preview" | "error";
...
// Pre-generate static bar heights so they look like a natural waveform
...
export default function VoiceRecordScreen({ navigation }: Props) {
  ...
  async function handleStop() {
    ...
    try {
      ...
      const res = await uploadVoice(patientId ?? "", uri);
      setVoiceId(res.voiceId);
      setPreviewUrl(res.previewUrl);
      setPhase("preview");

      if (res.previewUrl) {
        await autoPlay(res.previewUrl);
      }
    } catch {
      setPhase("error");
      setApiError(
  ...
  async function autoPlay(uri: string) {
    if (soundRef.current) {
  ...
  // ── Active bars = proportional to elapsed time ────────────────────────────
  const activeBars = phase === "recording"
            ...
            <View style={styles.previewRow}>
              ...
              <View style={styles.previewInfo}>
                <Text style={styles.previewTitle}>
                  "Good morning, Dad. It's day 1."
                </Text>
                <Text style={styles.previewSub}>
                  {previewUrl
                    ? isPlaying
                      ? "Playing cloned voice…"
                      : "Sarah's voice · cloned"
                    : "Voice clone ready"}
                </Text>
              </View>
            </View>
                ...
                <View
                  ...
                  style={[
                    styles.waveBarSmall,
                    {
                      ...
                      backgroundColor: i < 16 ? C.accent : C.hairline,
                    },
                  ]}
                />
              ))}
      ...
      <View style={styles.footer}>
        ...
        {(phase === "idle" || phase === "recording") && (
          <View style={styles.recordRow}>
            ...
            >
              {/* Square stop icon inside the circle */}
              {phase === "recording" ? (
