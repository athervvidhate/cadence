...
async function streamTextAudioController(req, res, next) {
  ...
  let voiceId = getBodyString(req.query.voiceId);

  // Allow patientId as an alternative to voiceId
  if (!voiceId) {
    const patientId = getBodyString(req.query.patientId);
    if (patientId) {
      voiceId = await getPatientVoiceId(patientId);
    }
  }

  if (!voiceId || !text || !isSupportedLanguage(language)) {
    const error = new Error('Query params (voiceId or patientId), text, and language ("en" | "es") are required.');
    error.statusCode = 400;
    throw error;
  }
